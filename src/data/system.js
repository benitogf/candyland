...
// Dependency helpers. Runs are always driven by real Claude Code — there is no
// demo/simulated mode — so "can this machine run agents?" is just whether the
// claude dependency is installed.
export const depByName = (system, name) => (system?.deps || []).find((d) => d.name === name) || null
export const claudeReady = (system) => !!depByName(system, 'claude')?.installed

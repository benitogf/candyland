...
// Setup & status: the detected platform, dependency state, executor mode, and
// per-platform install commands. Everything the user needs to go from "demo" to
// real runs, with copy-paste commands.
        ...
        <DialogContent dividers sx={{ borderColor: 'divider' }}>
            {!reachable ? (
                <Alert severity="error" variant="outlined" action={<Chip label="Retry" size="small" onClick={onRetry} sx={{ cursor: 'pointer' }} />}>
                    Can't reach the candyland server (localhost:8888). Start it, then retry — see the README "Run" section.
                </Alert>
            ) : !system ? (
                ...
                <>
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                        ...
                        <Chip size="small" variant="outlined" color={system.simulated ? 'warning' : 'success'} label={system.simulated ? 'executor · simulated' : 'executor · claude (real)'} />
                    </Box>

                    {system.simulated && (
                        <Alert severity="warning" variant="outlined" sx={{ mb: 2 }}>
                            Runs are <strong>simulated</strong> — a faithful demo, but no real code changes. Install Claude Code (below) to run real agents.
                        </Alert>
                    )}

...
// every level (runs · quests) and persist across a pivot — they live in the URL
// query string (see Tasks.js). Each control writes one query key via
// onChange(key, value); an empty value clears that key.
//
// Filters: text · no-parent / by-quest (parent) · by-repo/folder · by-status ·
// by-PR-state · by-date (from/to).
...
const FilterBar = ({ level, filters, runs, quests, onChange, onClear }) => {
    // Parent options depend on the level: runs can be filtered by a quest; quests
    // have no parent (the control hides).
    const questOpts = quests.map((q) => q.id)
    const showParent = level === 'runs'

    // Distinct repos/folders across the active level, for the repo dropdown.
    const source = level === 'runs' ? runs : quests
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, alignItems: 'center', mb: 2 }}>
            ...
            {showParent && (
                <Select label="Parent" value={filters.parent} onChange={(v) => onChange('parent', v)} width={170}>
                    ...
                    {questOpts.length > 0 && <MenuItem disabled value="__q">— quests —</MenuItem>}
                    {questOpts.map((id) => <MenuItem key={id} value={id}>{id}</MenuItem>)}
                </Select>
            )}

...
// Candyland observes runs — detritus creates and begins them over REST. A run
// briefly in "planning" (created, not yet begun) shows a transient starting view.
    ...
    const planning = run.status === 'planning'
        ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, py: 8 }}>
                <CircularProgress />
                <Typography variant="body2" color="text.secondary">Starting the run…</Typography>
            </Box>
        )
        : null

...
type Workspace struct {
	...
	// Deleted soft-deletes the workspace: the record (and its folders on disk) is
	// kept so the Tasks history can still show its name, but pickers hide it.
	// One-way — recreating the same id (same slug) clears it.
